...
const NFTFilterPanel = ({ nfts, onFilterChange }) => {
  ...
  // Calculate trait counts with proper BODY attributes matching your image
  const traitCounts = useMemo(() => {
    const counts = {};

    // Count traits from all NFTs (up to 10,000)
    nfts.forEach((nft) => {
    ...
    // For BODY attributes, ensure they match the exact order from your image
    // This helps with consistency across all 10,000 Kodas
    if (grouped.BODY) {
      const bodyOrder = [
        "BROWN",
        "GREY",
      ...
      // Reorder BODY traits according to the specified order
      const sortedBody = bodyOrder
        .map((trait) => {
          const found = grouped.BODY.find((t) => t.value === trait);
          return found || { value: trait, count: 0 };
        })
        .filter((t) => t.count > 0);

      grouped.BODY = sortedBody;
    }
